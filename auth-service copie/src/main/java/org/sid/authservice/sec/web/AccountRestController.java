...
public class AccountRestController {
    ...
    public AccountRestController(AccountServive accountServive) {
    ...
    public void addRoleToUser(@RequestBody UserRoleForm userRoleForm){
     accountServive.addRoleToUser(userRoleForm.getUsername(),userRoleForm.getRoleName());
    }

    @GetMapping(path = "/refreshToken")
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) throws  Exception{
      String authToken = request.getHeader(JWTUtil.AUTH_HEADER);
      if(authToken !=null && authToken.startsWith(JWTUtil.PREFIXE)){
          try {
              String jwt = authToken.substring(JWTUtil.PREFIXE.length());
